...
At page_size > 1 each branch's draft region is laid out holey per page and the
prefix partial-tail page is duplicated into it: page64 stays on spec v2 (overlap),
page4 runs on spec v1 (no overlap). flashinfer is pinned because this runs on the
cheap (5090) runner, where fa3 (Hopper-only) isn't available -- functional sanity
only, no perf/stress. (page>1 topk>1 on fa3 is covered on the Hopper runner in
